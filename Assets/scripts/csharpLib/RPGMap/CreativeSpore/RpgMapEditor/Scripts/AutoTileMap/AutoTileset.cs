...
namespace CreativeSpore.RpgMapEditor
{
	...
	public class AutoTileset : ScriptableObject 
	{
		...
		private void CreateAtlasMaterial()
		{
			string matPath = "";
#if UNITY_EDITOR
			Debug.Log("bbbb:",m_atlasTexture);
            Debug.Log("aaaaaaaa:" + AssetDatabase.GetAssetPath( m_atlasTexture ));
			...
			if( !string.IsNullOrEmpty( matPath ) )
			{
				...
				if( matAtlas == null )
				{
					matAtlas = new Material( Shader.Find("Custom/SimpleShaderCullOffMove2") ); //NOTE: if this material changes, remember to change also the one inside #else #endif below
...
#else
			AtlasMaterial = new Material( Shader.Find("Custom/SimpleShaderCullOffMove2") );
#endif
