...
namespace CreativeSpore.RpgMapEditor
{
	...
	public class AutoTileMapGui : MonoBehaviour
    {
        ...
        GUIContent[] scaleComboBoxList;
        private ComboBox scaleComboBoxControl;

        int m_ScaleSelectedIndex;
        float m_Camera2dDefualtSize;
        ...
        public void Init()
        {
            ...
            if (m_autoTileMap != null && m_autoTileMap.IsInitialized)
            {
                ...
                AutoTileMap.Instance.OnMapLoaded += (map) => { RefreshCombobox(); };
            }
        }
        ...
        public void RefreshCombobox()
        {
            ...
            scaleComboBoxList = new GUIContent[4];
            for (int i = 0; i < 4; i++)
            {
                scaleComboBoxList[i] = new GUIContent((i + 1) * 50 + "%");
            }
            ...
            m_autoTileMap.BrushGizmo.SelectedLayer = 0;
            comboBoxControl.SelectedItemIndex = m_autoTileMap.BrushGizmo.SelectedLayer;

            scaleComboBoxControl = new ComboBox(new Rect(0, 0, 150, 20), scaleComboBoxList[0], scaleComboBoxList, "button", "box", listStyle);
            scaleComboBoxControl.SelectedItemIndex = 1;
            m_Camera2dDefualtSize = m_camera2D.GetCameraOrthographicSize();
            #endregion
        }
        ...
        void Update()
        {
            ...
            bool isEKeyHold = Input.GetKey(KeyCode.Q);
            ...
            else if (m_focusWindow == eEditorWindow.MAPVIEW)
            {
                ...
                else
                {
                        ...
                        for (int tile_x = startTileX; tile_x <= endTileX; ++tile_x)
                        {
                            for (int tile_y = startTileY; tile_y <= endTileY; ++tile_y)
                            {
                                ...
                                else
                                {
                                    if (isEKeyHold)
                                    {
                                        int tileType = m_autoTileMap.GetAutoTile(tile_x, tile_y, m_autoTileMap.BrushGizmo.SelectedLayer).Id;
                                        m_autoTileMap.BrushGizmo.BrushAction.Push(m_autoTileMap, tile_x - m_dragTileX, tile_y - m_dragTileY, tileType, m_autoTileMap.BrushGizmo.SelectedLayer);
                                    }
                                    else
                                    {
                                        for (int i = 0; i < m_autoTileMap.GetLayerCount(); ++i)
                    ...
                    {
                        m_camera2D.AddCameraOrthographicSize(0.5f);
                    }
        ...
        void OnGUI()
        {
            ...
            float fPad = 4f;
            float fTilesetOffset = m_isLayersMenuHidden ? 64f : 160f;
            ...
            if (!m_isLayersMenuHidden)
            {
                ...
                MapLayerController.Instance.CheckLayerChange(m_autoTileMap.BrushGizmo.SelectedLayer);


                if (comboBoxControl.IsDropDownListVisible)
                {
                    scaleComboBoxControl.Rect.y = 99999;
                }
                else
                {
                    scaleComboBoxControl.Rect.y = 128;
                }
                scaleComboBoxControl.Rect.width = m_rTilesetRect.width;
                scaleComboBoxControl.Rect.height = 32;
                int selected = scaleComboBoxControl.Show();

                if (selected != m_ScaleSelectedIndex)
                {
                    m_camera2D.SetCameraOrthographicSize(((selected + 1) * 0.5f) * m_Camera2dDefualtSize);
                    m_ScaleSelectedIndex = selected;
                }
            }

            if (!comboBoxControl.IsDropDownListVisible && !scaleComboBoxControl.IsDropDownListVisible)
            {
