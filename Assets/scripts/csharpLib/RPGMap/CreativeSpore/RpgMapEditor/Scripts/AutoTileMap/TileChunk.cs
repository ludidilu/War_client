...
//
//     				for(int i = 0 ; i < m_animatedWaterfallTiles.Count ; i++)
//                {
//   					AnimTileData animTileData = m_animatedWaterfallTiles[i];
//  
...
//       		}
		...
		public void Configure(AutoTileMap autoTileMap, int layer, int startTileX, int startTileY, int tileChunkWidth, int tileChunkHeight)
        {
            ...
            if (MapLayerIdx == 0)
            {
                ...
                meshRenderer.sharedMaterial.SetFloat("fTextTileWidth", fTextTileWidth);
                meshRenderer.sharedMaterial.SetFloat("fTextTilePartHeight", fTextTilePartHeight);
            }
...
#if !UNITY_EDITOR
            if (MyAutoTileMap.MapLayers[MapLayerIdx].LayerType == eLayerType.Event)
            {
                return;
            }
#endif
        ...
        void FillData()
        {
            ...
            m_colors = new Color32[TileWidth * TileHeight * 4 * 4];
            m_uv = new Vector2[m_vertices.Length];
            m_uv2 = new Vector2[m_vertices.Length];
            m_triangles = new int[TileWidth * TileHeight * 4 * 2 * 3]; // 4 subtiles x 2 triangles per tile x 3 vertex per triangle
                        ...
                        for (int xf = 0; xf < 2; ++xf)
                        {
                            for (int yf = 0; yf < 2; ++yf)
                            {
                                ...
                                if (autoTile.Type == eTileType.OBJECTS || autoTile.Type == eTileType.NORMAL)
                                {
                                    ...
                                    m_uv2[vertexIdx + 0] = Vector2.zero;
                                    m_uv2[vertexIdx + 1] = Vector2.zero;
                                    m_uv2[vertexIdx + 2] = Vector2.zero;
                                    m_uv2[vertexIdx + 3] = Vector2.zero;
                                }
                                else
                                {
                                    ...
                                    if (MyAutoTileMap.Tileset.IsAutoTileAnimated(autoTile.Id))
                                    {
                                        ...
                                        m_uv2[vertexIdx + 0] = new Vector2(1, 0);
                                        m_uv2[vertexIdx + 1] = new Vector2(1, 0);
                                        m_uv2[vertexIdx + 2] = new Vector2(1, 0);
                                        m_uv2[vertexIdx + 3] = new Vector2(1, 0);
                                    ...
                                    else if (MyAutoTileMap.Tileset.IsAutoTileAnimatedWaterfall(autoTile.Id))
                                    {
                                        ...
                                        m_uv2[vertexIdx + 0] = new Vector2(2, subTileRow);
                                        m_uv2[vertexIdx + 1] = new Vector2(2, subTileRow);
                                        m_uv2[vertexIdx + 2] = new Vector2(2, subTileRow);
                                        m_uv2[vertexIdx + 3] = new Vector2(2, subTileRow);
                                    }
                                }
            ...
            System.Array.Resize(ref m_colors, vertexIdx);
            System.Array.Resize(ref m_uv, vertexIdx);
            System.Array.Resize(ref m_uv2, vertexIdx);
            System.Array.Resize(ref m_triangles, triangleIdx);
        }
    }
}
